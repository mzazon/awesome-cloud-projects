...
def lambda_handler(event, context):
    """
    AWS Lambda function to handle approved content workflow
    ...
    try:
        ...
        logger.info(f"Processing approved content: s3://{source_bucket}/{source_key}")
        
        # Get target bucket from environment
        target_bucket = os.environ['APPROVED_BUCKET']
        
        # Create target key with date-based partitioning
        target_key = f"approved/{datetime.utcnow().strftime('%Y/%m/%d')}/{source_key}"
        
        # Copy content to approved bucket with metadata
        ...
        # Send notification
        message = f"""
Content Moderation Result: APPROVED
...
Target Location: s3://{target_bucket}/{target_key}
        ...
        logger.info(f"Content approved and moved to: s3://{target_bucket}/{target_key}")
        
        return {
            'statusCode': 200,
            'body': json.dumps({
                'message': 'Content processed for approved workflow',
                'target_location': f's3://{target_bucket}/{target_key}'
            })
        }
        
    except Exception as e:
        logger.error(f"Error in approved handler: {str(e)}")
        return {
            'statusCode': 500,
